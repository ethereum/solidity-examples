...
import {PERF_LOGS, RESULTS_NAME_OPTIMIZED, RESULTS_NAME_UNOPTIMIZED} from "../constants";
...
export const printLatestDiff = (optimized: boolean = true): boolean => {
    const logFolders = indexedLogFolders(PERF_LOGS, 2);
    if (logFolders.length < 2) {
        return false;
    }
    const file = optimized ? RESULTS_NAME_OPTIMIZED : RESULTS_NAME_UNOPTIMIZED;
    const newLog = readLog(path.join(PERF_LOGS, logFolders[0]), file);
    const oldLog = readLog(path.join(PERF_LOGS, logFolders[1]), file);
