...
import {
    RESULTS_NAME_OPTIMIZED, RESULTS_NAME_UNOPTIMIZED, BIN_OUTPUT, TEST_FUN_HASH, TEST_LOGS
} from "./constants";
...
export const test = async (tests: Array<[string, string, string]>, optAndUnopt: boolean): Promise<boolean> => {
...
export const compileAndRunTests = async (units: Array<[string, string, string]>, optimize: boolean) => {
...
export const runTests = () => {
    ...
    for (const sigfile of sigfiles) {
        if (!isSigInHashes(BIN_OUTPUT, sigfile, TEST_FUN_HASH)) {
